(function($) {
	...
	function menuTriggerOpen(r) {
		if($(r).closest(".sub-menu").length>0){
			...
			$(".et-menu>li.menu-item.open").each(function(){
				menuTriggerClose($(this).find("button.open"));
			});
		}
	...
	$(document).ready(function(){
		if( $("body").hasClass("et-tb-has-header") ){
			$(".et_pb_menu__search-button").attr("aria-label", "open search form");
			$(".et_pb_menu__close-search-button").attr("aria-label", "close search form");
			
			$(".et_pb_menu__menu .menu-item-has-children > a").each(function(){
				var buttonHtml;
			...
			$(".et-menu.nav>li").on("keydown", function(n){
			...
			//submenu controls	
			$(".et-menu.nav .sub-menu li").on("keydown",function(e){
				e.stopPropagation();
				var activeElement = document.activeElement.tagName;
				var activeElClass = document.activeElement.className;
				...
				if( ( e.which==39 || e.which==40) && e.which!=16  ){//tab, right and down arrow
					e.preventDefault();
